...
void SearchThread::parseFile(std::string file, std::string target) {
    ...
    // used to avoid adding the header as the passage
    // for the first timestamp
    bool firstTimestamp = true;

    // if the subtitles are autogenerated the previously read timestamp
    // needs to be associated to the currently read passage
    ...
    while (std::getline(infile, line)) {
        ...
        if (isTime(line)) {
            ...
            // also ignore the subtitle header from appearing in results
            if (found && duration > 10 && firstTimestamp == false) {

                // create new match item
                videoMatch match(prevTimestamp, passage);
    ...
    // add the leftover passage with the last timestamp
    // for autogenerated subtitles
